...
export const getCreditStatsAction = userActionClient.action(async ({ ctx }) => {
  try {
    ...
    const expiringCreditsRows = await db
        .select()
      ...
      const totalExpiringCredits = expiringCreditsRows.reduce((sum, row) => sum + (row.remainingAmount ?? 0), 0);

    // totalExpiringCredits computed above
